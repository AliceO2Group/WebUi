 ...
 * @returns
 */
const configurationSelection = (workflow, configurations) => h('.w-100.flex-column.ph1', [
...
const configurationDropdownArea = (workflow, configurations) =>
  h('.dropdown-menu.w-100.scroll-y', { style: 'max-height: 25em;' }, configurations
    .filter((name) => name.toLocaleLowerCase().match(new RegExp(`.*${workflow.selectedConfigurationRaw.toLocaleLowerCase()}.*`))).map((name) =>
...
const loadErrorPanel = (workflow) => {
  ...
  if (isDetectorViewMatch) {
    return h('.w-100.flex-column.ph2', [h('.danger', 'Configuration cannot be loaded in this detector view')]);
 ...
 * @returns {vnode}
 */
const btnSaveEnvConfiguration = (model) => h('.flex-column.dropdown#flp_selection_info_icon', [
