...
export default (workflow, maxHeight) =>
  h('.w-100', [
    h('.w-100.flex-row.panel-title.p2', [
      ...
      h('h5.bg-gray-light', { style: 'width: 90%' }, workflow.flpSelection.list.kind === 'Success'
      ...
      h('.flex-column.dropdown#flp_selection_info_icon', { style: 'display: flex' }, [
        ...
        h('.p2.dropdown-menu-right#flp_selection_info', { style: 'width: 350px' }, 'Keep SHIFT / ⇧ key pressed when selecting two machines to select all in-between machines.'),
      ]),
    ]),
    h(
      '.w-100.p2.panel',
      workflow.flpSelection.list.match({
        ...
        Failure: (error) => h('.f7.flex-column', [
          error.includes(404) ?
            h('', error)
            :
            h('', [h('', ' Please use `Advanced Configuration` panel to select your FLP Hosts')]),
        ]),
      }),
    ),
  ]);
 ...
 * @returns {vnode}
 */
const flpSelectionArea = (list, workflow, maxHeight = 34.2) => list.length === 0 ? h('.f7.flex-column', 'Please select detector(s) first')
  :
