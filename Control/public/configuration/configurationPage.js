...
const expertPanel = (model, options) => h('.pv3', {
  ...
  h('.flex-column.mv3', [
    ...
    h('.panel.p2', [
      h('.flex-row.w-100', [
        ...
        inputNumberBox(model, 'Trigger Window Size', 0, Math.pow(2, 31 - 1), 'trigger-window-size', 'Size of the trigger window in GBT words'),
    ...
    h('.panel.p2', [
      ...
      h('.flex-row.w-100.p1', [
        ...
        h('.w-100.mh2', { style: 'display: flex; justify-content: space-between; flex-wrap: wrap;' }, [
          h('label.d-inline.f6.ph1', { style: 'white-space: nowrap', title: 'Toggle selection of all links' }, h('input', {
            type: 'checkbox',
  ...
  h('.flex-column.mv3', [
    ...
    h('.flex-row.w-100.p2.panel', [dropDown(model, 'Force config', ['TRUE', 'FALSE'], 'force-config', 'Flag to force configuration (needed to force the clock configuration)')]),
    h('.flex-row.w-50'),
    h('.flex-row.w-50'),
  ]),
    ...
    h('table.table.table-sm', [
      h('thead.panel-title', [
        h('tr', [
          ...
          h('th', { style: 'width:0' }, h('input.actionable-row', {
            type: 'checkbox',
          ...
          : Object.keys(readoutCardsMap).map((hostName) =>
            [
              h('tr', [
                ...
                h('td', { onclick: () => model.configuration.toggleAllReadoutCardsByHost(hostName) }, h('label.d-inline.actionable-row', { title: 'Select / Unselect all CRUs for this host' }, h('input.actionable-row', {
                  type: 'checkbox',
              ...
              readoutCardsMap[hostName].open && Object.keys(readoutCardsMap[hostName].objects).map((card) =>
                h('tr', { onclick: () => model.configuration.toggleReadoutCardSelection(hostName, card) }, [
                  ...
                  h('td', ''),
                  h('td', ''),
                  h('td', h('label.d-inline.actionable-row', { title: 'Select / Unselect this CRU' }, h('input.actionable-row', {
                    type: 'checkbox',
