 ...
 * @returns {vnode}
 */
export const header = (model) => [h('.w-100 text-center', [h('h4', 'Environments')])];
...
const requestsTable = (model, requests) =>
  h('table.table', [
    h('thead', [
      ...
      h('tr', [
        ['ID', 'Detectors', 'Workflow', 'Created by', 'When', 'State', 'Message', 'Action'].map((header) =>
    ...
    h('tbody', [
      requests.map((item) => h('tr', { style: { background: item.failed ? 'rgba(214, 38, 49, 0.2)' : '' } }, [
        ...
        h('td', { style: 'text-align: center;' }, item.detectors && item.detectors.length > 0 ? item.detectors.sort().join(' ') : '-'),
        h('td', { style: 'text-align: center;' }, item.workflow.substring(item.workflow.lastIndexOf('/') + 1, item.workflow.indexOf('@'))),
    ...
    h('tbody', [
      list.map((item) => {
        ...
        return h('tr', {
          ...
          h('td', { style: 'text-align: center;' }, h('a', {
...
const runColumn = (item, model) => {
  ...
  return h('td', { style: 'text-align: center;' }, h('.badge.f4', { class: classes }, text));
};
 ...
 * @returns {vnode}
 */
const actionsCell = (environment) => h('.btn-group', [
 ...
 * @returns {boolean}
 */
export const isGlobalRun = (vars) => vars['trg_enabled'] === 'true' && vars['trg_global_run_enabled'] === 'true';
