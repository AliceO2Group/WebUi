...
const tasksPerFlpTables = (environmentModel, environment) => {
  ...
  return [
    Object.keys(tasksByFlp).map((host) =>
      h('', [
...
const tasksPerEpnTables = (envModel, environment) => {
  ...
  return [
    Object.keys(tasksByHosts).map((host) =>
      h('', [
        h('.p2.flex-row.bg-primary.white', [h('h5.w-100', host)]),
        h('table.table.table-sm', { style: 'margin-bottom: 0' }, [
          h(
            'thead',
            h('tr', [['ID', 'Path', 'Ignored', 'State'].map((header) => h('th', header))]),
          ),
          h('tbody', [
            tasks.map((task) => {
              if (task.host === host) {
 ...
 * @returns {vnode}
 */
const showEnvTasksTable = (environment, tasks) => h('.scroll-auto.panel', [
  h('table.table.table-sm', { style: 'margin-bottom: 0' }, [
    h(
      'thead',
      h('tr', [
