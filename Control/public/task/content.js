...
const detectorPanels = (model, detectors) => [
  Object.keys(detectors)
    ...
    .map((detector) => h('.w-100', [
      ...
      detectors[detector].isOpened && h('.panel', [tasksTables(model, detectors[detector].list.payload)]),
    ])),
];
        ...
        h('table.table', {
          ...
          h(
            ...
            h(
              ...
              h('th.flex-row', { style: { 'justify-content': 'flex-end' }, colspan: 1 }, h('a', {
...
const cleanTasksButton = (task) =>
  h('.flex-column.dropdown#flp_selection_info_icon', { style: 'display: flex' }, [
    ...
    h('.p2.dropdown-menu-right#flp_selection_info.text-center', { style: 'width: 350px' }, 'Shutdowns or kills any task that is unlocked and not part of an active environment'),
  ]);
