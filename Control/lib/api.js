...
const logger = require('@aliceo2/web-ui').LogManager.getLogger(
  `${process.env.npm_config_log_label ?? 'cog'}/api`
);
...
// middleware
const {
  addDetectorIdMiddleware,
} = require('./middleware/addDetectorId.middleware.js');
const {
  logDeploymentRequestMiddleware,
} = require('./middleware/logDeploymentRequest.middleware.js');
const {
  minimumRoleMiddleware,
} = require('./middleware/minimumRole.middleware.js');
const {
  requireDetectorOrGlobalRoleMiddleware,
} = require('./middleware/requireDetectorOrGlobalRole.middleware.js');
const {
  validateConsulServiceMiddlewareFactory,
} = require('./middleware/validateConsulServiceMiddlewareFactory.js');
...
// controllers
const {
  QCConfigurationController,
} = require('./controllers/QCConfiguration.controller.js');
const { ConsulController } = require('./controllers/Consul.controller.js');
const {
  DeploymentController,
} = require('./controllers/Deployment.controller.js');
const {
  EnvironmentController,
} = require('./controllers/Environment.controller.js');
...
} = require('./controllers/WorkflowTemplate.controller.js');
...
} = require('./services/environment/EnvironmentCache.service.js');
...
} = require('./services/WorkflowTemplate.service.js');
const {
  QCConfigurationService,
} = require('./services/QCConfiguration.service.js');
...
const O2_CONTROL_PROTO_PATH = path.join(
  __dirname,
  './../protobuf/o2control.proto'
);
const O2_APRICOT_PROTO_PATH = path.join(
  __dirname,
  './../protobuf/o2apricot.proto'
);
...
module.exports.setup = (http, ws) => {
  ...
  const environmentCacheService = new EnvironmentCacheService(
    broadcastService,
    eventEmitter
  );

  const qcConfigurationService = new QCConfigurationService(consulService);
  const qcConfigurationController = new QCConfigurationController(
    qcConfigurationService,
    config.consul
  );
  ...
  const ctrlService = new ControlService(
    ...
    consulController,
    config.grpc,
    O2_CONTROL_PROTO_PATH
  );
  ctrlService.setWS(ws);
  const apricotProxy = new GrpcServiceClient(
    config.apricot,
    O2_APRICOT_PROTO_PATH
  );
  ...
  const workflowService = new WorkflowTemplateService(
    ctrlProxy,
    apricotService
  );
   ...
   */
  const envCtrl = new EnvironmentController(
    environmentService,
    workflowService,
    lockService,
    detectorService
  ...
  if (config.kafka && config.kafka?.enable) {
    try {
      ...
      aliEcsSynchronizer = new AliEcsSynchronizer(
        kafkaClient,
        cacheService,
        eventEmitter
      );
  ...
  const setDetectorsFromEnvironmentMiddleware =
    setDetectorsFromEnvironmentMiddlewareFactory(environmentService);
  const verifyLockOwnershipMiddleware =
    getDetectorsLockOwnershipMiddlewareFactory(lockService);
  const validateConsulServiceMiddleware =
    validateConsulServiceMiddlewareFactory(consulService);

  ctrlProxy.methods.forEach((method) =>
    http.post(`/${method}`, coreMiddleware, (req, res) =>
      ctrlService.executeCommand(req, res)
    )
  );

  http.get(
    '/workflow/template/default/source',
    workflowController.getDefaultTemplateSource.bind(workflowController)
  );
  http.get(
    '/workflow/template/mappings',
    workflowController.getWorkflowMapping.bind(workflowController)
  );
  http.get(
  ...
  http.get(
    '/runs/calibration',
    runController.getCalibrationRunsHandler.bind(runController)
  );

  http.get(
    '/environments',
    coreMiddleware,
    envCtrl.getEnvironmentsHandler.bind(envCtrl),
    { public: true }
  );
  http.get(
    '/environment/:id/:source?',
    coreMiddleware,
    envCtrl.getEnvironmentHandler.bind(envCtrl),
    { public: true }
  );
  http.post(
    '/environment/auto',
    coreMiddleware,
    envCtrl.newAutoEnvironmentHandler.bind(envCtrl)
  );
  http.put(
    '/environment/:id',
    coreMiddleware,
  ...
  http.delete(
    '/deploy/:id',
    minimumRoleMiddleware(Role.DETECTOR),
    deploymentController.acknowledgeDeploymentFailureHandler.bind(
      deploymentController
    )
  );

  http.post('/core/environments/configuration/save', (req, res) =>
    apricotService.saveCoreEnvConfig(req, res)
  );
  http.post('/core/environments/configuration/update', (req, res) =>
    apricotService.updateCoreEnvConfig(req, res)
  );
  ...
  apricotProxy.methods.forEach((method) =>
    http.post(`/${method}`, (req, res) =>
      apricotService.executeCommand(req, res)
    )
  );
  http.get('/core/detectors', (req, res) =>
    apricotService.getDetectorList(req, res)
  );
  http.get('/core/hostsByDetectors', (req, res) =>
    apricotService.getHostsByDetectorList(req, res)
  );

  http.post('/execute/o2-roc-config', coreMiddleware, (req, res) =>
    ctrlService.createAutoEnvironment(req, res)
  );
  ...
  http.put(
    `/locks/force/:action/${DetectorId.ALL}`,
    minimumRoleMiddleware(Role.ADMIN),
    addDetectorIdMiddleware(DetectorId.ALL),
    lockController.actionForceLockHandler.bind(lockController)
  );
  http.put(
    '/locks/force/:action/:detectorId',
    minimumRoleMiddleware(Role.GLOBAL),
    lockController.actionForceLockHandler.bind(lockController)
  );
  ...
  http.get(
    '/status/consul',
    statusController.getConsulStatus.bind(statusController)
  );
  http.get(
    '/status/grafana',
    statusController.getGrafanaStatus.bind(statusController)
  );
  http.get(
    '/status/notification',
    statusController.getNotificationSystemStatus.bind(statusController)
  );
  http.get(
    '/status/gui',
    statusController.getGuiStatus.bind(statusController),
    { public: true }
  );
  http.get(
    '/status/apricot',
    statusController.getApricotStatus.bind(statusController)
  );
  http.get(
    '/status/core',
    coreMiddleware[0],
    statusController.getAliECSStatus.bind(statusController)
  );
  http.get(
    '/status/system',
    statusController.getSystemCompatibility.bind(statusController)
  );
  http.get(
  ...
  http.get(
    '/consul/flps',
    validateConsulServiceMiddleware,
    consulController.getFLPs.bind(consulController)
  );
  http.get(
    '/consul/crus',
    validateConsulServiceMiddleware,
    consulController.getCRUs.bind(consulController)
  );
  http.get(
    '/consul/crus/config',
    validateConsulServiceMiddleware,
...
function initializeIntervals(
  ...
  intervalsService.register(
    statusService.retrieveConsulStatus.bind(statusService),
    SERVICES_REFRESH_RATE
  );
  intervalsService.register(
    statusService.retrieveAliEcsCoreInfo.bind(statusService),
    SERVICES_REFRESH_RATE
  );
  intervalsService.register(
    statusService.retrieveApricotStatus.bind(statusService),
    SERVICES_REFRESH_RATE
  );
  intervalsService.register(
    statusService.retrieveGrafanaStatus.bind(statusService),
    SERVICES_REFRESH_RATE
  );
  intervalsService.register(
    statusService.retrieveSystemCompatibility.bind(statusService),
    SERVICES_REFRESH_RATE
  );
  intervalsService.register(
