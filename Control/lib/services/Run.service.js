...
class RunService {
   ...
   * @returns {Promise<Object<String(Detector), Map<String(RunType), Object(CalibrationConfiguration, RunSummary, RunSummary)>.Error>} - list of calibration runs grouped by detector
   */
   ...
   * @returns {Promise<Object<String, CalibrationConfiguration.Error>} - map of calibration configurations
   * @example
  ...
  async _retrieveCalibrationConfigurationsForDetectors() {
    try {
      ...
      this._logger.errorMessage(
        `Unable to load calibration mapping due to: ${err}`,
   ...
   * @returns {Object<string, number>} - mapping of run types to their bookkeeping ID
   */
  get runTypes() {
