...
export class CentralConnection {
  ...
  /**
   * Sends an event to the central system via the gRPC stream.
   * @param data Message with event type and corresponding payload
   * @returns
   */
  public sendEvent(data: DuplexMessageModel) {
    if (!this._stream) {
      this._logger.warnMessage(`Stream is not defined. Connect to Central System first.`);
      return false;
    }

    try {
      this._stream?.write(data);
      this._logger.infoMessage(`Sent event to Central System:`, data);
      return true;
    } catch (err) {
      this._logger.errorMessage(`Error sending to Central System:`, err);
      return false;
    }
  }
