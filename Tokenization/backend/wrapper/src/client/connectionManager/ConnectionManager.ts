...
export class ConnectionManager {
  ...
  public async createNewConnection(address: string, direction: ConnectionDirection, jweToken?: string) {
    ...
    conn = new Connection(jweToken ?? '', address, direction, this.renewToken.bind(this));
  ...
  /**
   * Retrieves a connection instance by its token.
  ...
  /**
   * Sends command to central system to get all tokens
  ...
  public getAllTokens(): void {
    this._centralConnection.sendEvent({
      event: DuplexMessageEvent.MESSAGE_EVENT_GET_ALL_TOKENS,
    });
  }

  /**
   * Sends command to central system to renew token for a specific target connection
  ...
  private renewToken(expiredToken: string, targetAddress: string): void {
    const conn = this._sendingConnections.get(targetAddress);
    if (!conn) {
      return;
    }
  ...
  /**
   * Sends alert to the central system.
