...
export class GRPCAuthInterceptor {
  ...
  public async validate(
    ...
    let conn = this.connectionManager.getConnectionByAddress(
      clientAddress,
    ...
    if (conn) {
      ...
      if (conn.getToken() === jweToken) {
        ...
        const isReqAllowed = GRPCAuthInterceptor.isRequestAllowed(
        ...
        if (!isReqAllowed.isAllowed) {
          return this.alertAndFail(
              ...
              : TokenAuthReason.PERMISSION_EXPIRED,
            {
              method: String(call.request?.method || "POST").toUpperCase(),
              peer: clientAddress,
              peerSN: GRPCAuthInterceptor.normalizeSerial(
        ...
        if (
          !GRPCAuthInterceptor.isSerialNumberMatching(
            conn.getCachedTokenPayload(),
            peerCert
          )
        ) {
          ...
          return this.alertAndFail(
            ...
            TokenAuthReason.SERIAL_MISMATCH,
            {
              ...
              tokenSN: GRPCAuthInterceptor.normalizeSerial(
                conn.getCachedTokenPayload()?.sub
              ),
              token: jweToken,
              url: call.request?.path,
              status: grpc.status.UNAUTHENTICATED,
      ...
      conn = await this.connectionManager.createNewConnection(
        clientAddress,
        ConnectionDirection.RECEIVING,
        jweToken
      );
    ...
    try {
      ...
      privateKey = await importPKCS8(
        this.securityContext.clientPrivateKey.toString("utf-8"),
        "RSA-OAEP-256"
      );
    ...
    try {
      ...
      const jwk = {
        kty: "OKP",
        crv: "Ed25519",
        x: Buffer.from(this.securityContext.JWS_PUBLIC_KEY, "base64").toString(
          "base64url"
        ),
      };
    ...
    if (!isReqAllowed.isAllowed) {
      return this.alertAndFail(
          ...
          : TokenAuthReason.PERMISSION_EXPIRED,
        {
          ...
          status: grpc.status.UNAUTHENTICATED,
          addr: clientAddress,
          exp: (conn.getCachedTokenPayload() as TokenPayload)?.exp?.[
            String(call.request?.method || "POST").toUpperCase()
          ],
          ms: Date.now(),
        }
      );
   ...
   */
  public static isRequestAllowed(
    tokenPayload: TokenPayload | undefined,
    request: any
  ): { isAllowed: boolean; isUnexpired: boolean } {
    const method = String(request?.method || "POST").toUpperCase();
    const isValidPayload = GRPCAuthInterceptor.validateTokenPayload(
      tokenPayload,
      request.method
    );
   ...
   */
  private static validateTokenPayload(
    tokenPayload: TokenPayload | undefined,
    method: string
   ...
   */
  public static isSerialNumberMatching(
    tokenPayload: TokenPayload | undefined,
    peerCert: any
  ): Boolean {
    const clientSN = GRPCAuthInterceptor.normalizeSerial(
      peerCert?.serialNumber
    );
