...
export class RevokeTokenHandler implements CommandHandler<RevokeTokenCommand> {
  ...
  async handle(command: RevokeTokenCommand): Promise<void> {
    const { targetAddress, connectionDirection } = command.payload.singleToken || {};
    if (!targetAddress || !connectionDirection) {
      throw new Error('Target address and connection direction are required to revoke token.');
    }

    const conn = this.manager.getConnectionByAddress(targetAddress, connectionDirection);
