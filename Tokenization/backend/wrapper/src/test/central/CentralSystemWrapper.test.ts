...
describe('CentralSystemWrapper', () => {
  ...
  test('should log error if bind fails', () => {
    ...
    expect(logger.errorMessage).toHaveBeenCalledWith('Server bind error:', error);
  });
  ...
  test('should handle client stream events', () => {
    ...
    expect(logger.errorMessage).toHaveBeenCalledWith('Stream error from client client123:', expect.any(Error));
  });
});
