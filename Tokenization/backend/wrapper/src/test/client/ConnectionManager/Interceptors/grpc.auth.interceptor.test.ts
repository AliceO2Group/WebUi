  ...
  () => {
    return {
      Connection: jest
        .fn()
        .mockImplementation(
          (jweToken: string, address: string, direction: any) => {
            return {
              jweToken,
              address,
              direction,
...
describe("gRPCAuthInterceptor", () => {
  ...
  beforeEach(() => {
    ...
    isRequestAllowedSpy = jest
      .spyOn(interceptor, "isRequestAllowed")
      .mockImplementation((_p, _r, _cb) => true);

    isSerialNumberMatchingSpy = jest
      .spyOn(interceptor, "isSerialNumberMatching")
      .mockImplementation((_p, _pc, _cb) => true);

    getPeerCertFromCallSpy = jest
      .spyOn(interceptor, "getPeerCertFromCall")
      .mockReturnValue({ serialNumber: "DDEEFF" });
  });

  const getCreatedConn = () => {
    const instances = (Connection as jest.Mock).mock?.instances ?? [];
    return (
      instances.find((i: any) => i.address === MOCK_ADDRESS) ??
      mockClientConnections.get(MOCK_ADDRESS)
    );
  };
  ...
  it("should authenticate instantly if connection exists and token hasn't changed", async () => {
    const existingConn = new (Connection as jest.Mock)(
      VALID_JWE,
      MOCK_ADDRESS,
  ...
  it("should successfully authenticate a NEW connection", async () => {
    ...
    expect(created!.handleSuccessfulAuth).toHaveBeenCalledWith(
      DECRYPTED_PAYLOAD
    );
