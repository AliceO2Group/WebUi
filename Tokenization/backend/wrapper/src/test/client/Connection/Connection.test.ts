...
import { TOKEN_REASON_HEADER, TokenAuthReason, TokenPayload } from '../../../models/token.model';
...
describe('Connection', () => {
  ...
  beforeEach(() => {
    connection = new Connection(jweToken, targetAddress, direction, mockRenewToken, 'serial-123');
    mockRenewToken.mockClear();
  });

  it('should initialize with correct values', () => {
    expect(connection.token).toBe(jweToken);
    expect(connection.targetAddress).toBe(targetAddress);
    expect(connection.status).toBe(ConnectionStatus.CONNECTED);
    expect(connection.serialNumber).toBe('serial-123');
    expect(typeof connection.lastActiveTimestamp).toBe('number');
  });

  it('should update and get serial number', () => {
    connection.serialNumber = 'new-serial';
    expect(connection.serialNumber).toBe('new-serial');
  });
  ...
  it('should handle token revocation', () => {
    ...
    expect(connection.status).toBe(ConnectionStatus.UNAUTHORIZED);
  });
  ...
  it('should handle successful authentication', () => {
    ...
    expect(connection.cachedTokenPayload).toBe(payload);
    expect(connection.status).toBe(ConnectionStatus.CONNECTED);
    expect(typeof connection.lastActiveTimestamp).toBe('number');
  });
  ...
  it('should attach grpc client', () => {
    ...
    // peerClient is private, so we just verify the method doesn't throw
  ...
  describe('fetch', () => {
    ...
    it('should throw if peerClient is not attached', async () => {
      const c = new Connection(jweToken, targetAddress, direction, mockRenewToken);
    ...
    it('should handle token renewal and retry fetch', async () => {
      ...
      grpcClient.Fetch.mockImplementation((_req: any, meta: any, cb: any) => {
        ...
        if (callCount === 1) {
          ...
          err.metadataMap = new Map([[TOKEN_REASON_HEADER, TokenAuthReason.PERMISSION_EXPIRED]]);
          cb(err);
        } else {
    ...
    it('should throw on non-renewable error', async () => {
      grpcClient.Fetch.mockImplementation((_req: any, _meta: any, cb: any) => {
        ...
        err.metadataMap = new Map([[TOKEN_REASON_HEADER, TokenAuthReason.PERMISSION_FORBIDDEN]]);
        cb(err);
      });
      await expect(connection.fetch({ path: '/fail' })).rejects.toThrow('Forbidden');
    });
  });
  ...
  describe('createSslTunnel', () => {
    ...
    it('should create ssl tunnel and set status', () => {
      const grpcCreds = {};
      const oldCreateSsl = jest.spyOn(require('@grpc/grpc-js').credentials, 'createSsl').mockReturnValue(grpcCreds as any);
