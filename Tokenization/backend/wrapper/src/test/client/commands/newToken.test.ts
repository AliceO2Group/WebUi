 ...
 */
const createEventMessage = (targetAddress: string, connectionDirection: ConnectionDirection, token: string): Command => {
  return {
...
describe('NewTokenHandler', () => {
  ...
  it('should update token on existing SENDING connection', async () => {
    ...
    const conn = new Connection('old-token', targetAddress, ConnectionDirection.SENDING, null as any);
    conn.createSslTunnel(peerCtor, getTestCerts());

    (manager as any).sendingConnections.set(targetAddress, conn);

    const handler = new NewTokenHandler(manager);
    const command = new NewTokenCommand(createEventMessage(targetAddress, ConnectionDirection.SENDING, 'new-token').payload);
  ...
  it('should create new RECEIVING connection if not found', async () => {
    ...
    const handler = new NewTokenHandler(manager);
    const command = new NewTokenCommand(createEventMessage(targetAddress, ConnectionDirection.RECEIVING, 'test-token').payload);
  ...
  it('should handle DUPLEX direction by updating/creating both connections', async () => {
    ...
    const handler = new NewTokenHandler(manager);
    const command = new NewTokenCommand(createEventMessage(targetAddress, ConnectionDirection.DUPLEX, 'new-token').payload);
