...
http.get('/tokens', (req, res) => {
    // Fake long page load
    setTimeout(
        () => res.status(200).json([...fakeTokens.values()]),
        1000
    );
}, {public: true});

http.get('/tokens/:tokenId', (req, res) => {
...
http.get('/tokens/:tokenId/logs', (req, res) => {
    const tokenId = parseInt(req.params.tokenId, 10);

    // Artificially add an error
    if (tokenId === 3) {
        res.status(500).json({error: `An error occurred when trying to load logs for token ${tokenId}`});
        return;
    }
    ...
    if (!logs) {
        res.status(404).json({error: `No logs found found for token ${tokenId}`});
        return;
    }
