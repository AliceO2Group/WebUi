...
import { useServiceRoutesQuery, useRouteServiceOptionsQuery } from '~/feature/service-routes/api/queries';
...
export default function ServiceRoutesOverviewRoute() {
	...
	const serviceFromOptions = useMemo(() => {
		const base = serviceFromQueryEnabled ? serviceFromOptionsQuery.data ?? [] : [];
		if (serviceFromValue && !base.includes(serviceFromValue)) {
			return [serviceFromValue, ...base];
		}
		return base;
	}, [serviceFromOptionsQuery.data, serviceFromQueryEnabled, serviceFromValue]);

	const serviceToOptions = useMemo(() => {
		const base = serviceToQueryEnabled ? serviceToOptionsQuery.data ?? [] : [];
		if (serviceToValue && !base.includes(serviceToValue)) {
			return [serviceToValue, ...base];
		}
		return base;
	}, [serviceToOptionsQuery.data, serviceToQueryEnabled, serviceToValue]);
	...
	const handleSwapServices = useCallback(() => {
		...
		setServiceSelectInputs({
			serviceFrom: serviceToValue ?? '',
			serviceTo: serviceFromValue ?? '',
		});
	...
	const handleRouteCreateSubmit = useCallback((values: ServiceRouteCreationFormValues) => {
		...
		showModal({
			...
			content: (
				<Stack spacing={1}>
					<Typography variant="body2">
						Do you want to allow <strong>{serviceFrom}</strong> to reach <strong>{serviceTo}</strong>?
					</Typography>
			...
			onConfirm: () => {
				...
				registerRoute(
					{ serviceFrom, serviceTo, permissions },
					{
						onSuccess: () => {
							...
							render={({ field, fieldState }) => (
								<Autocomplete
									...
									onChange={(_, value) => {
										...
										handleServiceInputChange('serviceFrom', value ?? '');
									}}
						...
						<Controller
							...
							render={({ field, fieldState }) => (
								<Autocomplete
									...
									onChange={(_, value) => {
										field.onChange(value);
										handleServiceInputChange('serviceTo', value ?? '');
									}}
