...
//         {colsHeader: this.defaultUserConfig, criterias: this.defaultCriterias}});
//     }
//     const profileQuery = "SELECT * FROM profiles WHERE profile_name=?";
//     let profile_id;
//     await this.sqliteDb.query(profileQuery,[profile],true).then((profile) => {
//         if(profile) {
...
//   }
  ...
  async getProfile(req, res) {
    const profile = req.query.profile;
    if (!profile.trim()) {
        log.warn(`User profile ${profile} not passed, sending default instead`);
        res = this.getDefaultProfile(profile);
    } else {
        const profileQuery = "SELECT * FROM profiles WHERE profile_name=?";
        const configQuery = "SELECT * FROM columns INNER JOIN filters USING (profile_id) WHERE profile_id= (SELECT profile_id FROM profiles WHERE profile_name=?)";
        let profile_id;
        await this.sqliteDb.query(profileQuery,[profile],true).then((profile) => {
            if(profile) {
                profile_id = profile.profile_id;
        ...
        if(profile_id) {
            await this.sqliteDb.query(configQuery, [profile_id],true).then((config) => {
                if(config) {
