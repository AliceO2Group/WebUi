...
describe('Zoom test-suite', async () => {
  ...
  describe('Keyboard shortcuts', () => {
    it('should zoom in with Ctrl+Plus (Windows/Linux)', async () => {
    ...
    it('should zoom out with Ctrl+Minus (Windows/Linux)', async () => {
      await page.keyboard.down('Control');
    ...
    it('should zoom in with Meta+Plus (macOS)', async () => {
      ...
      assert.strictEqual(result.zoomLevel, 1.1, 'Meta++ should zoom in');
    });

    it('should zoom out with Meta+Minus (macOS)', async () => {
      await page.keyboard.down('Meta');
      await page.keyboard.press('Minus');
      await page.keyboard.up('Meta');
      ...
      assert.strictEqual(result.zoomLevel, 1, 'Meta+- should zoom out');
    });
  });

  describe('Mouse wheel', () => {
    it('should zoom in with Ctrl+ScrollUp (Windows/Linux)', async () => {
    ...
    it('should zoom out with Ctrl+ScrollDown (Windows/Linux)', async () => {
      await page.evaluate(() => {
        window.model.zoom.lastScrollTime = 0;
      });
    ...
    it('should zoom in with Meta+ScrollUp (macOS)', async () => {
      const container = await page.$('.logs-container');
      const box = await container.boundingBox();
      const x = box.x + box.width / 2;
      const y = box.y + box.height / 2;
      ...
      const afterZoomIn = await page.evaluate(() => window.model.zoom.level);
      assert.strictEqual(afterZoomIn, 1.1, 'Meta+scroll up should zoom in');
    });

    it('should zoom out with Meta+ScrollDown (macOS)', async () => {
      await page.evaluate(() => {
        window.model.zoom.lastScrollTime = 0;
      });
      ...
      const afterZoomOut = await page.evaluate(() => window.model.zoom.level);
      assert.strictEqual(afterZoomOut, 1, 'Meta+scroll down should zoom out');
    });
  });
  ...
  describe('Visual consistency', () => {
    it('should maintain row height ratio relative to font size across zoom levels', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const result = await page.evaluate(() => {
          const { fontSize } = window.model.zoom;
          const { rowHeightRem: rowHeight } = window.model.zoom;
