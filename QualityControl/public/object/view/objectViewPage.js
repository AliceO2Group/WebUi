...
function getActionsHeader(model) {
  return h('', { style: 'display: flex' }, [
    ...
    h('.text-center.flex-column', { style: 'flex-grow:1' }, [
      ...
      h('.w-100.flex-row', { style: 'justify-content: center' }, h('.w-40.p2.f6', timestampSelectForm(model))),
    ]),
    h('.flex-row', [
...
function getRootObject(model) {
  return h('.text-center', { style: 'flex-grow: 1; height:0;' }, model.router.params.objectName ?
 ...
 * @param {string} errorMessage - message to be displayed to the user
 * @returns {vnode} - virtual node element
 */
