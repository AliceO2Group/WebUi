...
export const filterTests = async (url, page, timeout = 5000, testParent) => {
  ...
  await testParent.test('filter should persist between pages', { timeout }, async () => {
    ...
    await page.locator('.root-tree div.object-selectable:last-child').click(); // This will select an object
  ...
  await testParent.test('should list all objects when disabling objectFilters', { timeout }, async () => {
    ...
    await extendTree(2, 4);
    let rowCount = await page.evaluate(() => document.querySelectorAll('ul >li').length);
    ...
    await extendTree(2, 4);
    rowCount = await page.evaluate(() => document.querySelectorAll('ul >li').length);
  ...
  await testParent.test('ObjectShow should only list versions based on the filter', { timeout }, async () => {
    ...
    await page.waitForFunction(() => document.querySelectorAll('#ObjectPlot select option').length >= 2);
  ...
  await testParent.test('ObjectTree infoPanel should show filtered object versions', { timeout }, async () => {
    ...
    await page.locator('.object-tree-leafObject:last-child .object-selectable').click();
    ...
    await page.locator('.object-tree-leafObject:nth-child(1) .object-selectable').click();
    await page.waitForSelector(versionsPath);
  ...
  async function extendTree(startIndex, subtreeCount) {
    await page.waitForSelector('.root-tree div.object-selectable:last-child');

    for (let i = startIndex; i < subtreeCount; i++) {
      await page.locator('.root-tree div.object-selectable:last-child').click();
      await page.waitForFunction((count) => document.querySelectorAll('ul > li').length > count, {}, i);
    }
  }
};
