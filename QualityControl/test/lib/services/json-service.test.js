...
describe('JSON file custom database', () => {
  ...
  describe('Creating a new Layout', () => {
    it('should throw an error if layout id is not provided', () => assert.rejects(async () => {
    ...
    it('should successfully create a new layout', () => assert.doesNotReject(async () => {
    ...
    it('should throw an error when creating a new layout with the same id as an existing layout', () => assert.rejects(async () => {
      await jsonConfig.createLayout(TEST_LAYOUT);
  ...
  describe('Reading/Updating/Deleting a Layout', () => {
    ...
    it('should throw an error if no layout was found by an id', () => assert.rejects(async () => {
      await jsonConfig.readLayout(111);
    }, new Error('layout (111) not found')));

    it('should throw an error when trying to update an inexistent layout by id', () => assert.rejects(async () => {
    ...
    it('should throw an error when trying to delete an inexistent layout by id', () => assert.rejects(async () => {
      await jsonConfig.deleteLayout(111, TEST_LAYOUT);
    }, new Error('layout (111) not found')));

    it('should successfully delete a layout by id', () => assert.doesNotReject(async () => {
  ...
  describe('Listing all existing layouts', () => {
    it('should successfully list all existing layouts with no filter', () => assert.doesNotReject(async () => {
    ...
    it('should successfully list all existing layouts based on give filter', () => assert.doesNotReject(async () => {
  ...
  describe('Testing read/write to fs', () => {
    it('should reject when layouts are missing from data with error of bad data format ', async () => assert.rejects(async () => {
      jsonConfig.data = '{}';
      await jsonConfig._writeToFile();
      await jsonConfig._readFromFile();
    }, new Error(`DB file should have an array of layouts ${CONFIG_FILE}`)));

    it('should reject when there is no data with error of bad data format ', async () => assert.rejects(async () => {
      jsonConfig.data = '';
      await jsonConfig._writeToFile();
      await jsonConfig._readFromFile();
    }, new Error(`DB file should have an array of layouts ${CONFIG_FILE}`)));

    it('should reject when data.layouts is not an Array with error of bad data format ', async () => assert.rejects(async () => {
    ...
    it('should successfully read layouts from data', async () => assert.doesNotReject(async () => {
      jsonConfig.data = { layouts: [] };
      await jsonConfig._writeToFile();
      await jsonConfig._readFromFile();
    }));

    it('should reject when there is missing data with error of bad JSON format ', async () => assert.rejects(async () => {
