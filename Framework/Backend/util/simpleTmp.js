 ...
 * @var CLEANUP_EVENT {string}      The event that is listened for when deleting the temporary root directory for the requests
 * @var _SERVER_URL {string}        The url used for downloading the root objects
 */
const DIR_PERMS = {
...
const CLEANUP_EVENT = 'exit';
const TAR_FILENAME = 'download';
const _SERVER_URL = 'http://ccdb-test.cern.ch:8080';
 ...
 * @return {void}
 */
function initTmpDir(path, callback) {
  ...
  fs.mkdir(TMP_DIR, DIR_PERMS.OWNER_RW && { recursive: true }, (err) => {
    ...
    prepareRootTmpRemovalOnSysExit(TMP_DIR, callback);
  });
}
 ...
 * @returns {void}
 */
function initNewRequestDir(request_id) {
 ...
 */
function scheduleRequestDirRemoval(dir) {
 ...
 * @returns {NodeJS.Process}    Returns the listener for the exit stored in the CLEANUP_EVENT variable
 */
function prepareRootTmpRemovalOnSysExit(path, callback) {
  return process.addListener(CLEANUP_EVENT, () => {
 ...
 */
async function retrieveFilesFromSubDir(request_id, callback) {
  ...
  if (matches_found != null && matches_found.length > 0) {
    return generateTarball(matches_found, `${path}/${TAR_FILENAME}.tar`);
  } else {
 ...
 */
function generateTarball(files, path) {
 ...
 */
async function sendDownloadRequest(object_id, request_id) {
  const url = `${_SERVER_URL}/download/${object_id}`;
 ...
 * @returns {void}
 */
async function sendDownloadRequests(object_ids, request_id) {
  for (const obj_id in object_ids) {
    await sendDownloadRequest(obj_id, request_id);
  }
}

async function returnTarballInRequestersSubDir(request_id) {
  return fs.readFileSync();
}

/**
 * Export functions that may be used by others
 * @exports initTmpDir(): {void}
 *  > Initialize the /tmp directory.
 *
 * @exports initNewRequestDir(request_id): {void}
 *  > Make new temporary subdirectory for a request.
 *
 * @exports sendDownloadRequest(object_id, request_id): {Promise<void>}
 *  > Requests root object to be downloaded from CCDB and stored at the temporary subdirectory made earlier.
 *
 * @exports retrieveFilesFromSubDir(request_id, callback): {Promise<Pack | void | any>}
 *  > Requests a tarball to be made of the request subdirectory and sends it as a filestream to the frontend.
 */
module.exports.initTmpDir = initTmpDir;

module.exports.initNewRequestDir = initNewRequestDir;

module.exports.sendDownloadRequest = sendDownloadRequest;

module.exports.sendDownloadRequests = sendDownloadRequests;

module.exports.retrieveFilesFromSubDir = retrieveFilesFromSubDir;
