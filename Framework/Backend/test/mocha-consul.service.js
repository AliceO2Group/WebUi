...
describe('Consul Service test suite', () => {
  describe('Check Initialization of ConsulService', () => {
    ...
    it('should throw error due to empty hostname in consul config', () => {
      assert.throws(() => {
        new ConsulService({});
      }, new Error('Hostname field cannot be empty'));
    });

    it('should throw error due to empty port in consul config', () => {
      assert.throws(() => {
        new ConsulService({ hostname: 'localhost' });
      }, new Error('Port field cannot be empty'));
    });

    it('should successfully create a consul connector', () => {
    ...
    it('should successfully set default values for API paths', () => {
      const consul = new ConsulService(config.consul);
  ...
  describe('Check `getConsulLeaderStatus()`', () => {
    const consul = new ConsulService(config.consul);
  ...
  describe('Check various ways of retrieving keys', () => {
    const consul = new ConsulService(config.consul);
    it('should successfully build a call to retrieve all keys', () => {
      nock('http://localhost:8080')
        .get('/v1/kv/?keys=true')
        .reply(200, ['a/b', 'b/c']);
    ...
    it('should successfully build the call to retrieve keys with a predefined keyPrefix', () => {
      nock('http://localhost:8080')
    ...
    it('should successfully build the call to retrieve keys with a predefined keyPrefix in format `/keyprefix` ', () => {
      nock('http://localhost:8080')
        .get('/v1/kv/keyprefix/?keys=true')
        .reply(200, ['keyprefix/somekey']);
      return consul.getKeysByPrefix('/keyprefix').then((res) => assert.deepStrictEqual(res, ['keyprefix/somekey']));
  ...
  describe('Check various ways of retrieving values based on key', () => {
    const consul = new ConsulService(config.consul);
    it('should successfully build a call to retrieve all keys', () => {
      nock('http://localhost:8080')
        .get('/v1/kv/?keys=true')
    ...
    it('should successfully build the call to retrieve a value by a key in format `key`', () => {
      nock('http://localhost:8080')
    ...
    it('should successfully build the call to retrieve all values with a keyprefix in format `/key/`', () => {
      nock('http://localhost:8080')
